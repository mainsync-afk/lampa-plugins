(function () {
    ...
    function enrichAndAdd(folder, card) {
        ...
        network.silent(url, function (data) {
            // Мержим локализованные данные поверх карточки из Trakt
            // Сохраняем ids и method из оригинальной карточки
    ...
    function applyTraktToLampa(folder, traktItems) {
        ...
        // Есть в Trakt, нет в Lampa -> добавить с обогащением через TMDB
        // Делаем последовательно чтобы не спамить TMDB запросами
        ...
        function addNext(index) {
            ...
            _suppressEvent = true;
            enrichAndAdd(folder, toAdd[index]);
            added++;
            // Небольшая задержка между запросами к TMDB
            setTimeout(function () {
                _suppressEvent = false;
                addNext(index + 1);
            }, 100);
