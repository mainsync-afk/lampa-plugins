(function () {
    ...
    function refreshCurrentFolder(folder) {
        try {
            ...
            // Пробуем мягкое обновление через render компонента
            // без полного перезапуска activity
            var component = active.activity;
            if (component && typeof component.rebuild === 'function') {
                component.rebuild();
            } else if (component && typeof component.create === 'function') {
                component.create();
            } else {
                // Крайний случай — полный перезапуск, но с небольшой задержкой
                // чтобы все Favorite.add успели завершиться
                setTimeout(function () {
        ...
        return new Promise(function (resolve) {
            ...
            function addNext(index) {
                if (index >= toAdd.length) {
                    ...
                    if (added || removed) refreshCurrentFolder(folder);
                    resolve();
                    return;
                }
