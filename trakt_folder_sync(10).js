(function () {
    ...
    function refreshCurrentFolder(folder, forceReplace) {
        try {
            var active = Lampa.Activity.active();
            if (!active || active.component !== 'favorite' || active.type !== folder) return;

            var comp = active.activity && active.activity.component;
            if (!forceReplace && comp && typeof comp.build === 'function') {
                // Мягкое обновление — только состояния карточек (значки закладок)
                var items = Lampa.Favorite.get({ type: folder }) || [];
                comp.build({ results: items });
            } else {
                // Полная перерисовка — нужна когда добавляются новые карточки
                // Небольшая задержка чтобы Lampa.Favorite успел сохранить все карточки
                _replacing = true;
                setTimeout(function () {
                    Lampa.Activity.replace(Object.assign({}, active, { page: 1 }));
        ...
        return new Promise(function (resolve) {
            ...
            function addNext(index) {
                if (index >= toAdd.length) {
                    ...
                    // Обновляем папку если она открыта
                    // forceReplace=true когда добавлены новые карточки
                    if (added) refreshCurrentFolder(folder, true);
                    else if (removed) refreshCurrentFolder(folder, false);
                    resolve();
                    return;
                }
            ...
            if (!toAdd.length) {
                ...
                if (removed) refreshCurrentFolder(folder, false);
                resolve();
            }
        });
    }
